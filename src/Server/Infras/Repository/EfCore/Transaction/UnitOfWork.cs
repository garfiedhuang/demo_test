namespace Hkust.Infras.Repository.EfCore.Transaction;

public abstract class UnitOfWork<TDbContext> : IUnitOfWork
    where TDbContext : DbContext
{
    protected TDbContext HkustDbContext { get; init; }
    protected IDbContextTransaction? DbTransaction { get; set; }

    public bool IsStartingUow => HkustDbContext.Database.CurrentTransaction is not null;

    protected UnitOfWork(TDbContext context)
    {
        HkustDbContext = context ?? throw new ArgumentNullException(nameof(context));
    }
    ...
    {
        if (HkustDbContext.Database.CurrentTransaction is not null)
            throw new ArgumentException($"UnitOfWork Error,{HkustDbContext.Database.CurrentTransaction}");
        else
